...
═══════════════════════════════════════
ПРАВИЛА ОПРЕДЕЛЕНИЯ SIM — ЧИТАЙ СТРОГО:
═══════════════════════════════════════
КЛЮЧЕВЫЕ ПРАВИЛА (применяй по порядку):

1. Если в тексте есть слово "sim" (или "сим") И слово "esim" (или "есим") одновременно
   → ВСЕГДА sim_esim. Примеры: "sim eSIM", "Sim+eSim", "сим + есим"

2. Если клиент написал "1sim", "1 sim", "1сим", "1 сим", "nano sim", "нано сим"
   (число 1 или слово "нано" перед sim, без упоминания eSIM отдельно)
   → ВСЕГДА sim_esim (на iPhone 1 физический слот = комплект SIM+eSIM)

3. Если клиент написал ТОЛЬКО "esim", "есим", "eSIM only", "2esim", "dual esim"
   (только виртуальный, БЕЗ слова "sim" рядом)
   → esim

4. Если клиент написал "2sim", "2сим", "dual sim", "два физических", "нано+нано"
   (явно два физических слота, БЕЗ упоминания esim)
   → sim_sim

5. Если слов SIM/eSIM/сим вообще нет в тексте → null
